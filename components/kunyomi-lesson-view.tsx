...
import type { KunyomiLessonDay } from "@/data/n1/kanji/lessons";

export function KunyomiLessonView({ lesson }: { lesson: KunyomiLessonDay }) {
  return (
    <div className="space-y-8">
          ...
          {lesson.entries.map((entry, i) => (
            <div key={i} className="rounded-2xl border border-border p-4 flex flex-col gap-2">
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black">{entry.word}</span>
      ...
      <KanjiExercises exercises={lesson.exercises} />
    </div>
  );
}
