...
import type { ChoiceQuestion, DragQuestion, GoiExercises as GoiExercisesData } from "@/data/n1/goi/lessons";
...
function DragQuestionCard({ q, index }: { q: DragQuestion; index: number }) {
  ...
  const blank = (
    <div
      ...
      onDrop={(e) => {
        ...
        if (value) setPlaced(value);
      }}
      className={cn(
        "min-w-[3rem] h-12 px-2 flex items-center justify-center rounded-xl border-2 border-dashed text-lg font-black",
  ...
  return (
    <div className="border-t border-border pt-4 first:border-t-0 first:pt-0">
      <p className="font-medium mb-3">
        {index + 1}. Bacaan: <span className="font-bold">{q.reading}</span> — seret atau klik jawaban yang tepat
      </p>
      ...
      <div className="flex items-center gap-2 mb-3">
        {q.blankPosition === "before" ? (
          <>
            {blank}
            <span className="text-2xl font-black">{q.fixedKanji}</span>
          </>
        ) : (
          <>
            <span className="text-2xl font-black">{q.fixedKanji}</span>
      ...
      <div className="flex flex-wrap gap-2">
        {q.choices.map((c) => (
          <div
            ...
            onClick={() => setPlaced(c)}
            className="px-3 h-10 flex items-center justify-center rounded-lg border-2 border-border bg-card text-base font-bold cursor-grab active:cursor-grabbing hover:border-primary/50 select-none"
          >
            {c}
          </div>
        ))}
...
export function GoiExercises({ exercises }: { exercises: GoiExercisesData }) {
  ...
  return (
    <Card className="p-6 bg-card">
      ...
      <div className="space-y-8">
        ...
        <div>
          <p className="text-sm text-muted-foreground mb-2">Bagian 2 — Lengkapi kata/frasanya</p>
          <div className="space-y-5">
