...
seperti seorang sensei yang peduli menyapa muridnya di tengah minggu.

Tulis SATU pesan chat pendek (2-4 kalimat) yang:
- Terasa personal, berdasarkan data di atas (progress, kesulitan, kebiasaan
  belajar) kalau ada. Kalau belum ada data sama sekali, sapa hangat dan ajak
...
async function generateBroadcastMessage(userId: string, userName: string, apiKey: string): Promise<string> {
  const systemPrompt = (await buildBaseSystemPrompt({ userName, userId })) + BROADCAST_INSTRUCTION;
...
export async function runLineBroadcast(): Promise<{ sent: number; skipped: number; errors: string[] }> {
  ...
  for (const user of candidates) {
    ...
    try {
      const message = await generateBroadcastMessage(user.id, user.name, apiKey);
